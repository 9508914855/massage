...
shareButton.addEventListener('click', () => { 
  ...
  const shareUrl = `${window.location.origin}${window.location.pathname}?message=${encodeURIComponent(messageElement.innerText)}&title=${encodeURIComponent(titleElement.innerText)}&token=${encodeURIComponent(token)}&key=${encodeURIComponent(key)}`; 
...
if (message && title && token && keyParam === key) { 
  // if a message, title, token, and key are present, check if the token is valid 
  ...
  if (viewedToken === null) { 
    ...
    // remove the token from local storage after it has been viewed once
    window.addEventListener('beforeunload', () => {
      localStorage.removeItem(token);
    });
  } else { 
    // if the token has already been viewed, show an error message
    messageElement.innerText = 'This message has already been viewed or the link is invalid.'; 
    titleElement.innerText = 'Error'; 
  } 
} else { 
  // if no message, title, token, or key are present, show an error message 
  messageElement.innerText = 'This link is invalid.'; 
  titleElement.innerText = 'Error'; 
}
